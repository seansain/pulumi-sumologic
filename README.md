...
First, clone this repo with the name of the desired provider in place of `sumologic`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-sumologic
```

Second, replace references to `sumologic` with the name of your provider:
...
The following configuration points are available for the `sumologic` provider:

- `sumologic:apiKey` (environment: `XYZ_API_KEY`) - the API key for `sumologic`
- `sumologic:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
